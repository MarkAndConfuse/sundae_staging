...
<div class="table-responsive">
    <table id="csd-table" class="table-docs table-bordered" width="100%" cellspacing="0">
        <thead>
            <tr>
            ...
            @foreach($assignCsd as $cSd)
            <tr>
                <td>{{ $cSd->account_id }}</td>
                <td>{{ $cSd->csd_name }}</td>
                <td>{{ $cSd->email }}</td>
                <td style="text-align: center;">
                    <button class="btn btn-danger btn-xs" data-csd-id="{{ $cSd->id }}" data-sub-id="{{ $cSd->sub_id }}" 
                    type="button" onclick="deleteCsd(this)"><i class="fa fa-trash"></i> Delete
                    </button>
                </td>
            </tr>
            @endforeach
